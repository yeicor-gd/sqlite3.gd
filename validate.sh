...
_extract_runtime_errors() {
    local log_file="$1"
    # handle_crash: detection removed because it seems to be randomly triggered even on empty projects?!
    grep -E -v "(ObjectDB|RID).*leaked|resources still in use at exit" "$log_file" | \
    grep -A 1 -E "^ERROR:|^SCRIPT ERROR:|^WARNING:|Shader compilation error|Script compilation error|Parse error|undefined method|undefined symbol|not found|No such|^TESTS FAILED" || return 0
}
